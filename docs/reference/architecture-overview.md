...
│  - Voice processing  │   │  - Profile metadata      │
...
Tasks and app state are **not stored in the Worker** — they live in the user's local IndexedDB and are synced peer-to-peer via encrypted Nostr events. The Worker serves PWA assets and handles Web Push delivery, authenticated voice processing, public link previews, NIP-05 lookups, and the independent Watch transport.
...
Agent commands arrive as JSON (`{v, id, op, params}`), are dispatched through `agentDispatcher.ts`, execute via `agentRuntime.ts`, and return `{v, id, ok, result, error}`. See `docs/reference/agent-mode.md` for the full command reference.
...
External agent sends JSON:
  ...
  └─ agentDispatcher.dispatchAgentCommand(jsonString)
       ...
       │    ... (see docs/reference/agent-mode.md for full list)
       │
...
See `docs/plans/engineering-roadmap.md` for the planned test expansion addressing the most critical gaps.
