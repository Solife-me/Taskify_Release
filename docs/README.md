# Documentation

Start with the [repository README](../README.md) for setup and the [contributor guide](../AGENT.md) for workflow and architecture.

Reference documents describe the system; plans track proposed or ongoing work; dated audits record findings at a point in time and may describe issues since resolved.

## Reference

- [Taskify Agent Mode](reference/agent-mode.md)
- [Cross-Device App State Sync](reference/app-state-sync.md)
- [Taskify Shared Architecture Boundaries](reference/architecture-boundaries.md)
- [Taskify Architecture Overview](reference/architecture-overview.md)
- [Bot Command Lists (NIP-51)](reference/bot-command-lists.md)
- [Cashu Wallet Layer](reference/cashu-wallet-layer.md)
- [Taskify Domains Layer Reference](reference/domains-layer-reference.md)
- [Taskify Functions and Flows](reference/functions-and-flows.md)
- [Native DM Push Relay Contract](reference/native-dm-push-relay.md)
- [Nostr Session Layer](reference/nostr-session-layer.md)
- [Worker Backend Layer (Cloudflare Worker)](reference/worker-backend.md)

## Plans and roadmaps

- [Native macOS Taskify](plans/native-macos.md)

- [Taskify iOS to Web-Wrapped PWA Plan](plans/2026-04-21-ios-webwrap.md)
- [Independent Apple Watch Chat Plan](plans/2026-08-31-independent-watch-chat.md)
- [iOS sharing and 500 MB attachments](plans/2026-09-02-ios-sharing-large-attachments.md)
- [CLI ↔ Core Integration Plan (runtime-nostr slice1)](plans/cli-core-integration-plan.md)
- [Engineering Roadmap — Documentation & Testing Initiative](plans/engineering-roadmap.md)
- [Native iOS Parity Execution Strategy (Formal)](plans/native-ios-parity-execution-strategy.md)
- [Taskify Native iOS Parity Plan (SwiftUI-First)](plans/native-ios-parity-plan.md)
- [Native iOS ↔ PWA Shell Parity Map (Historical Pre-Restart Record)](plans/native-ios-pwa-shell-parity-map.md)
- [Reliability & Offline-Sync TODO](plans/reliability-todo.md)
- [Voice Dictation — Implementation Plan](plans/voice-dictation-implementation-plan.md)

## Audits and dated investigations

- [Native board view performance — 2026-09-12](audits/native-board-performance-2026-09-12.md)
- [Nostr sync pipeline audit — September 3, 2026](audits/nostr-sync-audit-2026-09-03.md)
- [PWA / iOS task and DM history recovery](audits/pwa-client-history-sync-2026-09-11.md)
- [Solife physical-device performance audit — September 3, 2026](audits/solife-performance-audit-2026-09-03.md)
