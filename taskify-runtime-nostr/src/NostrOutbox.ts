 ...
 */
export function narrowRepublishedMutations(
  rows: NostrOutboxMutation[],
  options: { boardTag: string; keptRelayUrls: readonly string[]; legacyBurstMinimum?: number; legacyBurstGapMs?: number },
): { updated: NostrOutboxMutation[]; completedIds: string[] } {
  ...
  const boardRows = rows.filter((row) =>
    row.payload.event.tags.some((tag) => tag[0] === "b" && tag[1] === options.boardTag));

  const legacy = boardRows.filter((row) => row.isRepublish === undefined).sort((a, b) => a.intentAt - b.intentAt);
  const legacyBurstIds = new Set<string>();
  let run: NostrOutboxMutation[] = [];
  const closeRun = () => {
    if (run.length >= minimum) run.forEach((row) => legacyBurstIds.add(row.id));
    run = [];
  };
  for (const row of legacy) {
    if (run.length && row.intentAt - run[run.length - 1].intentAt > gap) closeRun();
    run.push(row);
  }
  closeRun();

  const updated: NostrOutboxMutation[] = [];
  const completedIds: string[] = [];
  for (const row of boardRows) {
    if (!(row.isRepublish === true || legacyBurstIds.has(row.id))) continue;
    const relays = normalizeRelayUrls(row.payload.relayUrls);
    if (!relays.some((relay) => kept.has(relay))) continue;
    const acked = new Set(normalizeRelayUrls(row.ackedRelays));
    const narrowed = relays.filter((relay) => kept.has(relay) || acked.has(relay));
    if (narrowed.length === relays.length) continue;
