...
type SubscriptionState = {
  ...
  pendingCursors: Map<string, { filter: NDKFilter; createdAt: number }>;
};
...
export class SubscriptionManager {
  ...
  private flushPending(state: SubscriptionState): void {
    ...
    if (this.subs.get(state.key) !== state) return;
    ...
    for (const { raw, relayUrl } of batch) {
      ...
      for (const filter of state.filters.filter((filter) => matchFilter(filter, raw))) {
        ...
        if (state.historyComplete) this.cursorStore.update(filter, newest, state.relayUrls);
      }
    }
  ...
  async subscribe(filtersInput: NDKFilter | NDKFilter[], options?: SubscribeOptions): Promise<ManagedSubscription> {
    ...
    sub.on("event", (evt: NDKEvent) => {
      ...
      if (!state.filters.some((filter) => matchFilter(filter, raw))) return;
      ...
      try { signatureValid = verifyEvent({ id: raw.id, pubkey: raw.pubkey, sig: raw.sig, kind: raw.kind, created_at: raw.created_at, tags: raw.tags, content: raw.content }); } catch { signatureValid = false; }
      if (!signatureValid) {
