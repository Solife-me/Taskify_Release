...
export async function recoverRelayHistory(session, filter, relay, onEvent, options = {}) {
    ...
    while (until >= since) {
        ...
        if (page.size >= effectiveLimit) {
            ...
            for (const id of nextBoundaryIDs)
                boundaryIDs.add(id);
